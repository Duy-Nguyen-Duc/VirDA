...
class DA_model_v4(nn.Module):
    def __init__(
        ...
        # Src: 0.95, Tgt: 0.46
        super(DA_model_v4, self).__init__()
    ...
    def forward(self, src_img, tgt_img, alpha, branch="da_train"):
        ...
        elif branch == "tgt_train": # student train
            # tgt_q is now src_img / tgt_k is now tgt_img
            freeze_layers([self.backbone, self.classifier])

            tgt_q_logits = self.classifier(self.backbone(self.visual_prompt(src_img)))
            tgt_k_logits = self.classifier(self.backbone(self.visual_prompt(tgt_img)))
            return tgt_q_logits, tgt_k_logits

        elif branch == "src_test": # teacher test
            src_feat = self.backbone(src_img)
            src_logits = self.classifier(src_feat)
            return src_logits

        elif branch == "tgt_test_stu": # student test
            fx = self.backbone(src_img)
            tgt_logits = self.classifier(fx)
            return tgt_logits
        
        elif branch == "tgt_test_tch":
            tgt_logits = self.classifier(self.backbone(self.visual_prompt(src_img)))
            return tgt_logits
